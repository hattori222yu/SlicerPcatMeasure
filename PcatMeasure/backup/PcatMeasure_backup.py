...
class PcatMeasure(ScriptedLoadableModule):
    def __init__(self, parent):
        ...
        parent.categories = ["Cardiology extension"]
...
class PcatMeasureWidget(ScriptedLoadableModuleWidget):
    def setup(self):
        ...
        self.showInflammationButton = qt.QPushButton("[5] : Show PCAT inflammation")
        ...
        arteryLayout.addStretch(1)   # 左寄せ＆幅安定
        ...
        formLayout.addRow(pcatTitle, qt.QWidget())  # 右側は空
...
#%% データ削除
    ...
    def onClearcenterlineNodes(self):
        scene = slicer.mrmlScene
        ...
        logging.info("All nodes except the segmentation have been cleared.")      
...
#%% inflammatinoをjpeg化する  
    def onShowInflammation(self):
        ...
        #
        coloredNode = self.createColorizedOriginal(
            volumeNode,
            segmentationNode,
            pcatSegID
        )
...
#%% selectbranchボタンを押したら 
    def onSelect_branches(self):
        ...
        safe_ct_name = self.sanitize_filename(self.ct_file_name)
        ...
        if(SAVE_INFO):
            ...
            for cell in self.cell_pt:
                ...
                self.coord_voxel[cell] = self.rasToIjkPoints(self.ctNode, ras_points)
...
#%% analysys pcatを押したら        
    ...
    def onAnalysys_pcat(self):
        ...
        self.subtractSegment(workSeg_sub,self.ctNode,segmentIdB,segmentIdA)
        ...
        self.intersectSegment(workSeg_int,self.ctNode,segmentIdB,segmentIdA)
        ...
        ######################################################
        self.PCAT_seg_filtered  = self.cloneSegmentation(PCAT_seg, "PCAT_filterd")        
        ...
        slicer.mrmlScene.RemoveNode(pcatarteryLabelNode)
