# -*- coding: utf-8 -*-
"""
Created on Fri Jan 16 07:31:37 2026
...
class CoronaryCenterlineCrossSectionWidget(ScriptedLoadableModuleWidget, VTKObservationMixin):
    def setup(self):
        ...
        self.kernelSizeSlider.value = 0.1
    ...
    def onPCATButtonClicked(self):
        ...
        slicer.util.selectModule('PCATMeasurement')
    
        # ここで必要なら、自動で対象セグメンテーションやCTを選択する
        # 
        # pcatModule = slicer.modules.PCATMeasurementWidget
        # pcatModule.setSegmentationNode(self.segNode)
        # pcatModule.setVolumeNode(self.ctSelector.currentNode())
        # モジュールを切り替え
        slicer.util.selectModule('Pcat_measure')
    
        # Widget を取得
        pcatWidget = slicer.modules.Pcat_measureWidget
        ...
        #slicer.mrmlScene.RemoveNode(slicer.mrmlScene.GetFirstNodeByClass("vtkMRMLMarkupsCurveNode"))
...
class CoronaryCenterlineCrossSectionLogic(ScriptedLoadableModuleLogic):
    ...
    def _updateClosedCircleFromSegmentation(self, segNode, segmentName,idx ,n=6,scaleFactor=1.5):
        ...
        if center is None:
            return
        print("center",center,"rdius",radius)
        # --- ClosedCurve node ---
        if not self.closedCurveNode:
            self.closedCurveNode = slicer.mrmlScene.AddNewNodeByClass(
                "vtkMRMLMarkupsClosedCurveNode",
